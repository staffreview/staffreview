...
description: One scout-agent unit of /staff-docs — reads an assigned slice of GitHub PR refs, judges each human review comment against the documentable-lesson criteria, and RETURNS candidate findings as JSON without posting, fetching commits, or modifying anything. A shared building block the /staff-docs orchestrator spawns per sub-agent; not usually run on its own.
---
