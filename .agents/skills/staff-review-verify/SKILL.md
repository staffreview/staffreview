...
description: One verify-agent unit of a staff-level code review — independently re-checks a batch of candidate findings against the code and RETURNS confirm/reject verdicts as JSON, so false positives never reach the author. A shared building block the /staff-review and /staff-loop orchestrators spawn per sub-agent; not usually run on its own.
---
