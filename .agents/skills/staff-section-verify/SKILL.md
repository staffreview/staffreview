...
description: One verify-agent unit of a /staff-section review — independently re-checks a batch of candidate findings against the actual files in the workspace and RETURNS confirm/reject verdicts as JSON, so false positives never reach the author. A shared building block the /staff-section orchestrator spawns per sub-agent; not usually run on its own.
---
