...
description: One find-agent unit of a /staff-section review — deep-reviews an assigned set of whole files from the current workspace (not a diff) across the staff review areas and RETURNS findings as JSON without posting. A shared building block the /staff-section orchestrator spawns per sub-agent; not usually run on its own.
---
