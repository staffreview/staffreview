...
test("openBrowser: CLI default, gear-menu toggle persists, and CLI setter writes it", async ({ page }) => {
  ...
  await expect.poll(async () => (await staff(["settings", "get", "openBrowser"])).trim()).toBe("false");

  expect((await staff(["settings", "set", "openBrowser", "true"])).trim()).toBe("openBrowser: true");
  await page.reload();
  await page.getByTestId("settings-menu-button").click();
  await expect(page.getByTestId("open-browser-auto")).toHaveAttribute("aria-checked", "true");
});
