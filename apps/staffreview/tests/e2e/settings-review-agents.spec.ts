...
test("reviewAgents: CLI default, gear-menu stepper persists, and the [1,20] clamp holds", async ({
  ...
  await page.getByTestId("review-agents-increase").click();
  await posted;
  ...
  await expect
    .poll(async () => (await staff(["settings", "get", "reviewAgents"])).trim())
    .toBe("3");

  // Out-of-range writes are clamped to [1, 20] server-side.
  const setAgents = async (v: number) => {
    const res = await page.request.post("/api/settings", { data: { reviewAgents: v } });
    return (await res.json()).settings.reviewAgents;
  };
  ...
  await page.reload();
  await page.getByTestId("settings-menu-button").click();
  await expect(page.getByTestId("review-agents-value")).toHaveText("1 agent");
});
