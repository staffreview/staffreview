...
test("comment --priority is stored, normalized, and validated (CLI)", async () => {
  ...
  const p2 = JSON.parse(
    await staff(["comment", "add", "--author", "cli", "--priority", "2", "--body", "Note."]),
  );
...
test("a prioritized comment shows its P-badge in the UI", async ({ page }) => {
  ...
  await staff([
    ...
    "P1",
    "--body",
    "This jumped 20 → 200 — confirm intentional.",
  ]);

  await expect(page.getByTestId("priority-P1").first()).toBeVisible();
});

test("a resolved thread shows its priority in the detail, not the summary row", async ({
  page,
}) => {
  await page.goto("/");
