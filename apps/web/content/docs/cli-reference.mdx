...
```bash
staff review post                      # infer the PR from gh or GitHub Actions
staff review post --pr 123             # explicitly select a PR
staff review post --slug main..WT      # post comments from a specific diff
staff review post --github-repo owner/repo --pr 123 --commit "$HEAD_SHA"
```

The command uses the GitHub CLI's existing authentication (`GH_TOKEN`,
`GITHUB_TOKEN`, or `gh auth login`). It posts root comments as one PR review,
rejects stale base/head context, and skips a review already posted for the same
commit. Replies and resolved-thread metadata remain local.
