...
By default it invokes `codex exec` and passes the review prompt on stdin. To use
another harness, pass `--review-command <cmd>` or set
`STAFF_WATCH_REVIEW_COMMAND`; the command receives the prompt on stdin plus
`STAFF_WATCH_PR_NUMBER`, `STAFF_WATCH_PR_URL`, `STAFF_WATCH_DIFF_SLUG`, and
`STAFF_WATCH_AGENTS` in the environment.
