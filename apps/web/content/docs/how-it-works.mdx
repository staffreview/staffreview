...
`staff install` writes eleven skills to `.agents/skills/` (symlinked into
`.claude/skills/` so Claude Code picks them up as slash commands) — eight you run
directly (`/staff-review`, `/staff-section`, `/staff-resolve`, `/staff-comment`,
`/staff-document`, `/staff-copy`, `/staff-docs`, `/staff-loop`) plus three building
blocks the orchestrators fan out to (`/staff-review-find`, `/staff-review-verify`,
`/staff-docs-scout`). `/staff-section` reuses the review find/verify pair in a
whole-file mode rather than its own.
...
gitignored automatically. See [The docs](/docs/docs).
