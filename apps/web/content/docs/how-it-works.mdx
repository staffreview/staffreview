...
`.claude/skills/` so Claude Code picks them up as slash commands). They’re just
Markdown — open `.agents/skills/staff-review/SKILL.md` and tune it to your
team’s standards. See the [Skills reference](/docs/skills) for what each one
does.

## The library

`.staffreview/library/` is your team’s captured review wisdom. `/staff-review`
reads it on every pass and re-flags any recurrence of a documented mistake. The
library is meant to be **committed**; the session data
(`.staffreview/diffs/`, `attachments/`, `active.json`) is gitignored
automatically. See [The library](/docs/library).
