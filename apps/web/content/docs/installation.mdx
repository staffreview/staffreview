...
- The [**GitHub CLI (`gh`)**](https://cli.github.com) — optional, and only for
  `/staff-docs`, which mines your merged PRs for review lessons. It must be
  installed and authenticated (`gh auth login`); without it, `/staff-docs` just
  skips PRs and still works on your local review history.

<Callout type="info">
  Once installed, run `staff install` inside a git repo to write the skills and
