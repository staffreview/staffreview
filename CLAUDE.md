...
**To change skill behavior, edit `.agents/skills/<name>/SKILL.md`** (the `skills/*.md` symlinks and the `SKILLS` map in `cli.ts` follow automatically). Orchestrators (`staff-review`, `staff-loop`, `staff-docs`) fan out to building-block skills (`staff-review-find`, `staff-review-verify`, `staff-docs-scout`); `staff-section` reviews whole files of the existing codebase a rotating section at a time (tracking progress in `.staffreview/section-cache.json`) and fans out to its own `staff-section-find`/`staff-section-verify` pair; `staff-copy` imports open GitHub PR review threads locally; `staff-comment` is the thin CLI wrapper they all post through.
